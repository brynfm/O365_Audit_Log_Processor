...
def date():
    ...
    expiry_date = todays_date + timedelta(days=45)
    print(f'Today date: {todays_date}')
...
if platform == 'Windows':
    base_path = os.path.join(base_path, "static'\\'assets'\\'csv_files\\")
    export_path = os.path.join(export_path, "static'\\'assets'\\'csv_export'\\'")
else:
    base_path = os.path.join(base_path, 'static/assets/csv_files/')
    export_path = os.path.join(export_path, 'static/assets/csv_export/')
...
# access_token = ['4b8a2af90147bson.errors.InvalidDocument: cannot encode object: datetime.date(2019, 7, 15), of type: <class 'datetime.date'>b5','d9c112d971b1bd','4daea41e191295']
...
def ip_info_func(ip_address,access_token):
    # Bryn
